...
class http_conn {
 ...
 private:
  ...
  HTTP_CODE parse_headers(char* text);       // 分析请求头部字段
  HTTP_CODE parse_content(char* text);       // 分析请求的消息体
 ...
 public:
  ...
  static int m_epollfd;     // epoll 内核事件表中
  static int m_user_count;  // 统计用户数量

 private:
  int m_sockfd;           // 当前 http 连接的本地 socket
  ...
  // 请求是否保持连接，保持则不关闭，只是重新初始化 http 对象
  bool m_linger;

  // 客户请求的目标文件呗mmap到内存的起始位置
  char* m_file_address;  // 目标文件的地址

  // 目标文件的状态，包括是否存在、是否为目录、是否可读、文件大小等
