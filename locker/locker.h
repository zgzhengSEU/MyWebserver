...
class sem {
 public:
  // sem_init/destory/wait/post 函数 成功返回0，失败返回-1
  sem() {
...
class locker {
 public:
  // pthread_mutex_函数，成功返回 0，失败返回 错误码
  locker() {
  ...
  // 互斥锁解锁，如果有其他线程正在等待这个互斥锁，则某一个线程将获得
...
class cond {
 public:
  // pthread_cond_init/destory/broadcast/signal/wait 函数
  // 成功返回 0，失败返回 错误码
  cond() {
  ...
  bool wait(pthread_mutex_t* m_mutex) {
    int ret = 0;
    // 调用pthread_cond_wait前，必须确保互斥锁mutex已经加锁
    ret = pthread_cond_wait(&m_cond, m_mutex);  //成功返回 0
    return ret == 0;  // 返回是否等到条件变量
  }
