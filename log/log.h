...
class Log {
 public:
  ...
  // 异步写日志公有方法，调用私有方法async_write_log写日志
  static void* flush_log_thread(void* args) { // 异步日志写入线程的回调函数
    Log::get_instance()->async_write_log();
  }

  // 日志的初始化，包括文件名、缓冲区大小、最大行数、最长日志队列
  bool init(const char* file_name, int log_buf_size = 8192,
            unsigned int max_lines = 5000000, int max_queue_size = 0);
  // 将输出内容按照标准格式整理
