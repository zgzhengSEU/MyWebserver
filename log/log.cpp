...
// 异步写需要设置队列
bool Log::init(const char *file_name, int log_buf_size, unsigned int max_lines,
               int max_queue_size) {
  // 使用队列则说明使用异步，队列大小为0说明使用同步
  if (max_queue_size >= 1) {
    m_is_async = true;
  ...
  m_log_max_lines = max_lines; //日志的最大行数
  ...
  if (p == NULL) {  // 如果只是一个文件名: "myFile"
    ...
    snprintf(log_full_name, 255, "%d_%02d_%02d_%s", my_tm.tm_yday + 1900,
...
void Log::write_log(int level, const char *format, ...) {
  ...
  if (m_today != my_time.tm_mday || m_count % m_log_max_lines == 0) {
    ...
    snprintf(tail, 16, "%d_%02d_%02d_", my_time.tm_year + 1900,
             my_time.tm_mon + 1, my_time.tm_mday);

    // 如果 m_today 过期（今天是新的一天而且还没有日志）
  ...
  // 将可变参数 vaList 按照 format 的格式保存到 m_buf + n 的位置，
  // 大小为m_log_buf_size - 1 ，返回写入字符的个数，失败返回负值
