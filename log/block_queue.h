...
class block_queue {
 public:
  ...
  bool pop(T& item) {
    ...
    // 如果 B 抢到了锁——消耗资源——释放锁
    // 而对于 A 来说，此时的他 wait 成功了，就是执行接下来的语句，
    // if 语句块之后的逻辑操作， 但是这个资源已经被 B 消耗了，
    // 所以 A 再操作这个资源是不存在的 所以应该使用 while，
    // 即使 A wait 成功了也要重新判断队列是否为空
    while (m_size <= 0) {
      // 在将线程加入线程等待队列时(cond_wait())，正在加入的线程是持有锁的，原因：
      // 如果加入过程中释放锁，则其他正在队列的线程可能此时接受到信号，开始抢锁使用资源，
      // 但是这时 A 还没进入队列一起抢资源，他必不可能抢到，
      // 所以对于还没有加入队列的 A 线程来说是不公平
      // 而A会在加入队列后释放锁（在这个过程中资源信号不会发生），然后大家一起公平等待资源信号
      if (!m_cond.wait(m_mutex.get())) {
        // 封装的 wait 的返回值是 bool，false 表示 wait 失败
        // 因为此时已经抢到锁，所以要先释放锁，然后返回 false
        m_mutex.unlock();
        return false;
      }
    }
